...
module.exports = class Product {
  constructor(title, price, description, rating, imageURL = defaultImageURL) {
    this.uuid = uuid.v4();
    this.title = title;
    this.price = price;
  ...
  save() {
    ...
    fs.readFile(productsPath, (err, fileContent) => {
      Product.getProductsFromFile(products => {
        const productIdx = products.findIndex(p => p.uuid === this.uuid);
        if (productIdx < 0) {
          products.push(this);
        } else {
  ...
  delete() {
    console.log("deleting product " + this.uuid);
    fs.readFile(productsPath, (err, fileContent) => {
      Product.getProductsFromFile(products => {
        const productIdx = products.findIndex(p => p.uuid === this.uuid);
        if (productIdx < 0) {
        ...
        Cart.deleteProduct(this.uuid, err => {
          fs.writeFile(productsPath, JSON.stringify(products), (err) => {
            console.log(err);
          });
        });
  ...
  static getProductsFromFile = cb => {
    fs.readFile(productsPath, (err, fileContent) => {
      ...
      cb(JSON.parse(fileContent).map(p => Product.fromJSON(p)));
    });
  }
  ...
  static findById(id, cb) {
    Product.getProductsFromFile(products => {
      const productIdx = products.findIndex(p => p.uuid === id);
      if (productIdx < 0) {
        return cb(null);
      }
      cb(products[productIdx]);
    });
  }
