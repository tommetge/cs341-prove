...
module.exports = class Product {
  ...
  delete() {
    return Product.deleteById(this.id);
  }
  ...
  static async save(product) {
    if (product._id) {
      return await Product.collection().replaceOne({ _id: product._id }, product);
    }

    return await Product.collection().insertOne(product);
  }

  static async fetchAll() {
    return await Product.collection().find().toArray();
  }

  static async findById(id) {
    return await Product.collection().find({ _id: new mongodb.ObjectId(id) }).next();
  }

  static async deleteById(id) {
    return await Product.collection().deleteOne({ _id: new mongodb.ObjectId(id) });
  }
}
